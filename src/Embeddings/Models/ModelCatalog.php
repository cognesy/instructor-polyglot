...
final class ModelCatalog
{
    private const array CONFIG_PATHS = [
        'config/embed/models',
        'packages/polyglot/resources/config/embed/models',
        'vendor/cognesy/instructor-php/packages/polyglot/resources/config/embed/models',
        'vendor/cognesy/instructor-polyglot/resources/config/embed/models',
    ];
    ...
    /** @var array<string, EmbeddingModel> */
    ...
    public function find(string $driver, string $model): EmbeddingModel
    {
        $key = self::lookupKey($driver, $model);

        return $this->resolved[$key] ??= $this->load($driver, $model)
            ?? EmbeddingModel::unknown($driver, $model);
    }
    ...
    private static function discoverForBasePath(string $basePath): self
    {
        ...
        return new self(...[...$paths, __DIR__.'/../../../resources/config/embed/models']);
    }

    private function load(string $driver, string $model): ?EmbeddingModel
    {
        ...
        foreach ($this->paths as $path) {
            ...
            if (! is_file($candidate) || ! is_readable($candidate)) {
                throw new RuntimeException("Embedding model record is not a readable file: {$relativePath}");
            }
    ...
    private function hydrate(string $relativePath, string $driver, string $model): EmbeddingModel
    {
        $data = $this->config?->loadRaw($relativePath)->toArray()
            ?? throw new RuntimeException('Embedding model catalog is unavailable.');
        self::assertFields($data, ['schemaVersion', 'version', 'profile'], 'embedding model record');
        if (($data['schemaVersion'] ?? null) !== 1) {
            throw new InvalidArgumentException('Unsupported embedding model record schemaVersion; expected integer 1.');
        }
        $version = $data['version'] ?? null;
        if (! is_string($version) || trim($version) === '') {
            throw new InvalidArgumentException('Embedding model record requires a non-empty version.');
        }
        $profileData = $data['profile'] ?? null;
        if (! is_array($profileData) || ($profileData !== [] && array_is_list($profileData))) {
            throw new InvalidArgumentException('Embedding model record profile must be an object.');
        }
        $profile = EmbeddingModel::fromArray($profileData, $version);
        if (! $profile->matches($driver, $model)) {
            throw new InvalidArgumentException("Embedding model record identity does not match path: {$relativePath}");
        }
