...
final readonly class JeffResponseAdapter implements DecisionResponseAdapter
{
    ...
    public function fromHttpResponse(HttpResponse $response, DecisionRequest $request): DecisionResponse
    {
        ...
        return $this->decoder->decode(
            wireAnswers: $this->requiredObject($body, 'answers', 'Jeff response answers'),
            request: $request,
            model: $this->requiredString($body, 'model', 'Jeff response model'),
    ...
    private function decodeBody(HttpResponse $response): stdClass
    {
        try {
            ...
            throw new DecisionResponseException('Jeff returned malformed JSON.', $response->statusCode());
        }
        if (! $body instanceof stdClass) {
            throw new DecisionResponseException(
                'Jeff response body must be a JSON object.',
                $response->statusCode(),
            );
        }
    ...
    {
        $usage = $this->requiredObject($body, 'usage', 'Jeff response usage');
    ...
    private function optionalNonNegativeInt(stdClass $object, string $field): ?int
    {
        ...
        if (! is_int($object->{$field}) || $object->{$field} < 0) {
            throw new DecisionResponseException(
                "Jeff usage field '{$field}' must be a non-negative integer or null.",
            );
        }
    ...
    private function assertJsonContentType(HttpResponse $response): void
    {
        ...
        if ($contentType === null || strtolower(trim(explode(';', $contentType, 2)[0])) !== 'application/json') {
            throw new DecisionResponseException(
                'Jeff response content type must be application/json.',
                $response->statusCode(),
            );
        }
    }
