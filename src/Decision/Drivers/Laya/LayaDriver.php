...
final readonly class LayaDriver implements CanProcessDecisionRequest
{
    ...
    public function __construct(
        ...
        $this->requestAdapter = $requestAdapter ?? new LayaRequestAdapter($config);
        $this->responseAdapter = $responseAdapter ?? new LayaResponseAdapter();
    }
    ...
    private static function providerError(?int $statusCode, ?string $retryAfter = null): DecisionProviderException {
        return match (true) {
            $statusCode === 401 || $statusCode === 403 => new DecisionAuthenticationException(
                "Laya authentication failed (HTTP {$statusCode}).",
                $statusCode,
                $retryAfter,
            ),
            $statusCode === 429 => new DecisionRateLimitException(
                'Laya rate limit exceeded (HTTP 429).',
                $statusCode,
                $retryAfter,
            ),
            $statusCode === null => new DecisionTransientException('Laya network request failed.'),
            $statusCode === 408 || $statusCode >= 500 => new DecisionTransientException(
                "Laya service is temporarily unavailable (HTTP {$statusCode}).",
                $statusCode,
                $retryAfter,
            ),
            default => new DecisionInvalidRequestException(
                "Laya rejected the request (HTTP {$statusCode}).",
                $statusCode,
                $retryAfter,
            ),
        };
